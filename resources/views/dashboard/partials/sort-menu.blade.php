@php
    /**
     * Selector de orden. Usa <details> nativo en vez de JavaScript: se abre y
     * se cierra con teclado sin código propio, y funciona aunque Vite no haya
     * compilado nada.
     */
...
<details class="relative">
    ...
    <div class="absolute right-0 z-10 mt-1.5 flex w-48 flex-col rounded-xl border border-ink-200 bg-white p-1 shadow-card">
        @foreach (DashboardSort::cases() as $case)
            <a href="{{ route('dashboard', ['filtro' => $filter->value, 'orden' => $case->value]) }}"
               @class([
