...
<div class="grid grid-cols-1 gap-3.5 sm:grid-cols-2 xl:grid-cols-4">
    ...
    <div class="{{ $card }}">
        ...
        <span class="{{ $note }}">Pendientes sin holgura: cualquier atraso mueve la entrega</span>
    ...
    <div class="{{ $card }}">
        ...
        @if ($summary['next_deadline'])
            ...
            <div class="mt-2 flex gap-1">
                <span class="{{ $pill }} {{ $next->isOverdue() ? 'bg-critical-soft text-critical' : 'bg-done-soft text-done' }}">
                    <span class="size-1.5 rounded-full bg-current"></span>
                    {{ $next->isOverdue() ? 'Atrasado '.$next->daysBehindSchedule().' d' : 'En plazo' }}
                </span>
            </div>
        @else
