@php
    /**
     * Barra lateral fija de 220 px, réplica de la del mockup (pantalla 02).
     * El listado de proyectos y el medidor de consultas salen del usuario
     * autenticado, así que la barra es la misma en todas las pantallas.
     */
    $user = auth()->user();
    $sidebarProjects = $user?->projects()->latest()->take(6)->get() ?? collect();
    ...
    // Clases literales: Tailwind escanea el archivo, así que no pueden armarse
    // concatenando el nombre del color en tiempo de ejecución.
    $toneDot = [
        'done' => 'bg-done',
...
<aside class="flex w-[220px] flex-none flex-col gap-1 border-r border-ink-200 bg-ink-50 p-2.5">
    ...
    @if ($sidebarProjects->isNotEmpty())
        ...
        @foreach ($sidebarProjects as $sidebarProject)
            @php
                $isCurrent = request()->route('project')?->is($sidebarProject) ?? false;
            @endphp
            ...
            <a href="{{ $sidebarProject->status === \App\Enums\ProjectStatus::Ready
               ...
               class="{{ $navItem }} {{ $isCurrent ? $navActive : $navIdle }}">
                {{-- El punto repite el estado que la tarjeta muestra con pastilla. --}}
    ...
    {{-- Medidor de la cuota mensual de generaciones con IA. --}}
